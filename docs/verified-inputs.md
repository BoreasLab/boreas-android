...
| Either descriptor route works, and the core closes neither | api/android.md#who-owns-the-file-descriptor, quoting both methods: `getFd()` leaves ownership with the `ParcelFileDescriptor`, `detachFd()` moves it to the caller's native code | `detachFd()`, into `boreas_tunnel_start_fd`, which api/android.md#hand-the-descriptor-over-instead documents as owning the descriptor on every path and closing it at `free`. The `ParcelFileDescriptor` is inert afterwards, so a double close is unreachable. |
| Closing a descriptor is not how a blocked read is unblocked | [`close(2)`, CAVEATS](https://man7.org/linux/man-pages/man2/close.2.html): "It is probably unwise to close file descriptors while they may be in use by system calls in other threads in the same process." | The core's concern now: it reads the descriptor through readiness, never a blocking read, and closes it after its reactor has stopped. Nothing in this app closes it. |
